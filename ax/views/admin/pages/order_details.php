    ...
    <div class="row-fluid sortable">		
        <div class="box span12">
            ...
            <div class="box-content">
                ...
                $xx = $this->db->select('*')->from('tbl_order_details')->where('order_id', $order_id)->get()->result();
                        foreach ($xx as $single_order_details) {
                            $i++;
                            ?>
                            <tr>
                                <td><?php echo $i; ?></td>
                                ...
                                <td>Tk.<?php echo $this->cart->format_number($single_order_details->product_price)?> </td>
                                <td><?php echo $single_order_details->product_sales_quantity ?></td>
                                 <td>
                                <?php
									$result = $this->db->get_where('tbl_order', ['order_id' => $single_order_details->order_id])->row();
									if ($result) {
									  echo 	$result->actions;									
									} else {
										echo "not found";
                                ...
                                <td>Tk.<?php echo $this->cart->format_number($single_order_details->product_price * $single_order_details->product_sales_quantity) ?></td>
                            </tr>
                        <?php } ?>
                    </tbody>
                    <tfooter>
                        <td colspan="6">Total Amount</td>
                        <td>= Tk.<?php echo $this->cart->format_number($order_info->order_total)?> </td>
                    </tfooter>
                </table>            
            </div>
        </div><!--/span-->
